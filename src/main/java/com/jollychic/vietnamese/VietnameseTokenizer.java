...
public class VietnameseTokenizer extends Tokenizer {
    ...
    private com.jollychic.vietnamese.Tokenizer tokenizer;
    ...
    public VietnameseTokenizer(boolean sentenceDetectorEnabled, boolean ambiguitiesResolved) {
        ...
        if (this.sentenceDetectorEnabled) {
            sentenceDetector = MySentenceDetectorFactory.create(IConstants.LANG_VIETNAMESE);
        }
        tokenizer = AccessController.doPrivileged(new PrivilegedAction<com.jollychic.vietnamese.Tokenizer>() {
            @Override
            public com.jollychic.vietnamese.Tokenizer run() {

                com.jollychic.vietnamese.Tokenizer tokenizer = TokenizerProvider.getInstance().getTokenizer();
